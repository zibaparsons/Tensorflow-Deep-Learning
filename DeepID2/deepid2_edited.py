...
with graph.as_default():
    ...
    with tf.variable_scope("DeepID2") as scope:
        CNN_output_Frontal, MLP_output_Frontal = deepid2_Architecture_Edited.MLP_Frontal(images_Frontal, dropout_param)
        print MLP_output_Frontal.get_shape()
        CNN_output_Profile, MLP_output_Profile = deepid2_Architecture_Edited.MLP_Profile(images_Profile, dropout_param)
...
with tf.Session(graph=graph) as sess:
    ...
    for epoch in range(num_epoch):
        ...
        for i in range(range_val):
            ...
            input1, input2, y_cont, y_f, y_p = deepid2_Architecture_Edited.get_batch_edited(start_idx, end_idx, X_train_main, X_train_aux, y_train, y_frontal_train, y_profile_train)
            ...
            #                                              dropout_param: 0.9})
            _, loss_value, predict = sess.run([optimizer, loss, distance],
                                              feed_dict={images_Frontal: input1, images_Profile: input2, labels: y_cont, label_F: y_f, label_P: y_p,
                                                         dropout_param: 0.9})
            ...
            distance_train_evaluation = distance.eval(
                feed_dict={images_Frontal: input1,
                           images_Profile: input2,    #images_Profile: X_train[start_idx:end_idx, :, :, num_channels:]
                           dropout_param: 1.0})
...
Plot_ROC_Fn(y_train, distance_train_output, 'Train', 'Output')#[:160]
